<div class="modal-dialog"
    ...
    <div class="modal-content">
        ...
        {!! Form::open([
            'url' => action([\App\Http\Controllers\Restaurant\ModifierSetsController::class, 'update'], [$modifer_set->id]),
            'method' => 'PUT',
            'id' => 'edit_form',
        ]) !!}
            ...
            <div class="row">
                ...
                <div class="col-sm-12">
                    <div class="form-group">
                        ...
                        {!! Form::text('name', $modifer_set->name, [
                            'class' => 'form-control',
                            'required',
                            ...
                            @foreach ($modifer_set->variations as $modifier)
                                <tr>
                                    <td>
                                        <div class="form-group">
                                            <input type="text"
                                                   name="modifier_name_edit[{{ $modifier->id }}]"
                                                   class="form-control"
                                                   value="{{ $modifier->name }}"
                                    ...
                                    <td>
                                        <div class="form-group">
                                            <input type="text"
                                                   name="modifier_price_edit[{{ $modifier->id }}]"
                                                   class="form-control input_number"
                                                   value="{{ @num_format($modifier->sell_price_inc_tax) }}"
                                                   placeholder="@lang('lang_v1.price')"
                                                   required>
                                        </div>
                                    </td>
                                    <td>
                                        @if (!$loop->first)
                                            <button class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-error pull-right remove-modifier-row"
                                                    type="button"><i class="fa fa-minus"></i></button>
                                        @else
                                            <button class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-primary pull-right add-modifier-row"
                                                    type="button"
                                                    data-html="{{ $html }}">
