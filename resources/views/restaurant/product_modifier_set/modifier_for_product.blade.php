...
<div>
    ...
    <i class="fa fa-external-link-alt cursor-pointer text-primary select-modifiers-btn"
       title="@lang('restaurant.modifiers_for_product')"
       data-toggle="modal"
       data-target="#{{ $id }}"></i>
</div>
<div class="modal fade modifier_modal"
     id="{{ $id }}"
     tabindex="-1"
     role="dialog">
    <div class="modal-dialog"
        ...
        <div class="modal-content">
                                    ...
                                    <div class="panel-body">
                                        <div class="btn-group"
                                            ...
                                            @foreach ($modifier_set->variations as $modifier)
                                                <label
                                                       class="btn btn-primary @if (!empty($edit_modifiers) && in_array($modifier->id, $product->modifiers_ids)) active @endif">
                                                    <input type="checkbox"
                                                           autocomplete="off"
                                                           value="{{ $modifier->id }}"
            ...
            <div class="modal-footer">
                <button data-url="{{ action([\App\Http\Controllers\Restaurant\ProductModifierSetController::class, 'add_selected_modifiers']) }}"
                        type="button"
                        class="tw-dw-btn tw-dw-btn-primary tw-text-white add_modifier"
                        data-dismiss="modal">
