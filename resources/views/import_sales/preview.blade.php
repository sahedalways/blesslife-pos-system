...
@section('content')
    ...
    <section class="content">
        {!! Form::open([
            'url' => action([\App\Http\Controllers\ImportSalesController::class, 'import']),
            'method' => 'post',
            'id' => 'import_sale_form',
        ]) !!}
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        {!! Form::select('group_by', $parsed_array[0], null, [
                            ...
                            @foreach (array_slice($parsed_array, 0, 101) as $row)
                                ...
                                @if ($loop->index == 0)
                                    <tr>
                                        ...
                                        @foreach ($row as $k => $v)
                                            <td>
                                                {!! Form::select('import_fields[' . $k . ']', $import_fields, $match_array[$k], [
                                                    'class' => 'form-control import_fields select2',
                                                    'placeholder' => __('lang_v1.skip'),
                                                    'style' => 'width: 100%;',
                                                ]) !!}
                                            </td>
                                        @endforeach
        ...
        <div class="row">
            <div class="col-md-12">
                <button type="submit"
                        class="tw-dw-btn tw-dw-btn-primary tw-text-white pull-right">@lang('messages.submit')</button>
            </div>
        </div>
        {!! Form::close() !!}
    </section>
...
@section('javascript')
    <script type="text/javascript">
        $(document).on('submit', 'form#import_sale_form', function() {
            ...
            if (import_fields.indexOf('customer_phone_number') == -1 && import_fields.indexOf('customer_email') == -
                1) {
