$(document).ready(function () {
    ...
    function setButtonState($btn, isLoading, originalText = null) {
        if (isLoading) {
            ...
            $btn.prop('disabled', false).html(textToRestore).css('color', '');
        }
    }
    ...
    $('.product-description-label').after(
        createAiButtonHtml('get_product_description', null, 'col-sm-4 text-right')
    );
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.success === true) {
                    ...
                    $(document)
                        .off('click', '#use_description')
                        .on('click', '#use_description', function () {
    ...
    $('.image-label').after(
        createAiButtonHtml('generate_product_image', null, 'col-sm-6 text-right')
    );
                ...
                if (response.success) {
                    ...
                    if (response.invoice_details) {
                        ...
                        if (response.invoice_details.supplier_id) {
                            ...
                            var supplierOption = new Option(
                                response.invoice_details.supplier_name,
                                response.invoice_details.supplier_id,
                                true,
                                true
                            );
                            ...
                            if (response.invoice_details.supplier_address) {
                                $('#supplier_address_div').html(
                                    response.invoice_details.supplier_address
                                );
                            }
                        }
                        ...
                        var purchaseDate = moment(
                            response.invoice_details.purchase_date,
                            'DD/MM/YYYY HH:mm'
                        ...
                        // Set purchase status
                        $('select[name="status"]')
                            .val(response.invoice_details.purchase_status)
                            .trigger('change');

                        // Set business location
                        $('select[name="location_id"]')
                            .val(response.invoice_details.location_id)
                            .trigger('change');
                    }
    ...
    $('#ai-analysis-container').append(
        createAiButtonHtml('pull-right', 'ai_profit_loss_analysis_btn')
    );
    ...
    function displayMissingSkuWarning(missingSkus) {
        var warningHtml =
            '<div class="py-2 px-3 mb-5 text-center">' +
    ...
    $('.diet-plan-button').after(
        createAiButtonHtml('generate_diet_plan', null, 'col-md-12 text-right')
    );
    ...
    $('.workout-plan-button').append(
        createAiButtonHtml('generate_workout_plan', null, 'col-md-12 text-right')
    );
    ...
    $('.message-container').after(
        createAiButtonHtml('generate_message_ai', null, 'col-md-4 text-right')
    );
